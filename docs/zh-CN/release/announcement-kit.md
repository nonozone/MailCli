...
- 先构建 `mailcli`
- 直接使用 `examples/config/fixtures-dir.yaml`
- 在不配 IMAP 的情况下跑本地 `sync -> threads -> thread` 闭环

当前 RC 的亮点：

- 将原始邮件解析为 `StandardMessage` JSON 和 Markdown
- 可以先通过内置 `dir` driver 走零网络本地工作流
...
- `README.md`
- `docs/zh-CN/examples/local-thread-demo.md`
...
现在你可以先用零网络本地 fixture 工作流体验它，再决定是否配置 IMAP。
...
现在可以先通过内置 `dir` driver 和仓库 fixture 零网络体验。
...
- 通过内置 `dir` driver 提供零网络本地 thread 工作流
- 基础 IMAP 读路径
- SMTP 发信路径
