...
- `mailcli reply --dry-run <reply.json>`
  也支持 `reply_to_id`，会通过配置好的 driver 自动解析线程头。

## 当前发送侧状态

- `pkg/composer` 已经可以在本地把 `DraftMessage` 和 `ReplyDraft` 编译成原始 MIME。
- `mailcli send` 在有账户配置时，已经可以把 MIME 交给 driver。
- `mailcli reply` 在有账户配置时，也已经可以把 MIME 交给 driver。
